                ...
                <div className="space-y-2.5 mt-1">
                  {(['provider', 'patient'] as const).map((type) => (
                    <label key={type} className="flex items-center gap-2.5 cursor-pointer">
                      ...
                      <span className="text-sm" style={{ color: LABEL_COLOR }}>{type === 'provider' ? 'Provider / Clinician' : 'Patient'}</span>
                    </label>
                  ))}
                </div>
              </div>
                ...
                <>
                  ...
                  <div>
                    <label className="flex items-start gap-3 cursor-pointer">
                      ...
                      <span className="text-sm leading-relaxed" style={{ color: LABEL_COLOR }}>I confirm that the information provided above is accurate to the best of my knowledge. {REQUIRED}</span>
                    </label>
                  </div>
                  {reporterType === 'provider' && (
                    <>
                  ...
                  <div className="pt-2"><button type="submit" className="btn-primary" disabled={submitting}>{submitting ? 'Submitting...' : 'Submit Outcomes Report'}</button></div>
                </>
              )}
            </form>
          </div>
