        ...
        <div className="container-xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-start">
            <div>
              ...
              <h2 className="text-3xl font-bold mt-2 mb-6" style={{ color: 'var(--navy)' }}>{content.structure.heading}</h2>
              <p className="text-base leading-relaxed mb-6" style={{ color: 'var(--text-mid)' }}>{content.structure.description}</p>

              <div className="rounded-2xl p-6 mb-5" style={{ background: 'var(--off-white)', border: '1px solid var(--border)' }}>
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-2 h-8 rounded-full" style={{ background: 'var(--gold)' }} />
                  <h3 className="font-bold text-lg" style={{ color: 'var(--navy)' }}>{content.structure.researchEntity.title}</h3>
                </div>
                <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--text-mid)' }}>{content.structure.researchEntity.description}</p>
              ...
              <div className="rounded-2xl p-6" style={{ background: 'var(--navy)', border: '1px solid rgba(200,149,44,0.2)' }}>
                ...
                <p className="text-sm leading-relaxed text-gray-300 mb-4">{content.structure.peptidePure.description}</p>
                <ul className="space-y-2">
            ...
            <div className="space-y-8">
              <div className="rounded-2xl overflow-hidden shadow-2xl w-full relative" style={{ height: '460px' }}>
                <Image src="/images/peptide-pure-1.webp" alt="PeptidePure™ clinical research facility" fill className="object-cover object-center" sizes="(max-width: 1024px) 100vw, 580px" />
              ...
              <div className="rounded-2xl p-8" style={{ background: 'var(--off-white)', border: '1px solid var(--border)' }}>
                ...
                <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--text-mid)' }}>{content.clinicalContext.intro}</p>
                ...
                <div className="px-4 py-3 rounded-xl mb-4" style={{ background: 'rgba(200,149,44,0.1)', border: '1px solid rgba(200,149,44,0.3)' }}>
                  <p className="text-sm font-semibold" style={{ color: 'var(--gold)' }}>{content.clinicalContext.solution}</p>
                </div>
                <p className="text-sm leading-relaxed" style={{ color: 'var(--text-mid)' }}>{content.clinicalContext.conclusion}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {content.principles.items.map((item: { icon: string; title: string; desc: string }, i: number) => (
              <div key={i} className="p-6 rounded-2xl text-center" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(200,149,44,0.2)' }}>
