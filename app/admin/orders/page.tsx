          ...
          {['all', ...STATUS_OPTIONS].map((s) => (
            <button
              ...
              style={{
                ...
                border: `1px solid ${statusFilter === s ? 'var(--navy)' : 'var(--border)'}`,
              }}
            >
              {s}
            </button>
      ...
      <div className="container-xl pb-12">
        {loading ? (
          ...
          <div className="py-20 text-center">
            <p className="text-sm" style={{ color: 'var(--text-light)' }}>No orders found.</p>
          </div>
        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {order.payment_method && (
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded-full" style={{ background: 'var(--off-white)', color: 'var(--text-light)' }}>
                            {order.payment_method === 'credit_card' ? 'Card' : order.payment_method}
                          </span>
                        )}
                      </div>
                      <p className="text-xs mt-1" style={{ color: 'var(--text-light)' }}>
                        {addr?.name || 'Unknown'} &middot; {formatDate(order.created_at)}
                      </p>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="px-5 pb-5 pt-1 border-t" style={{ borderColor: 'var(--border)' }}>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                        {/* Items */}
                        <div>
                          <h4 className="text-xs font-bold uppercase tracking-wide mb-3" style={{ color: 'var(--text-light)' }}>
                            Items
                          </h4>
                          <div className="space-y-2">
                            {order.items.map((item, i) => (
                              <div key={i} className="flex justify-between text-sm">
                                <span style={{ color: 'var(--text-dark)' }}>
                                  {item.product_name} <span style={{ color: 'var(--text-light)' }}>x{item.quantity}</span>
                                </span>
                            ...
                            <div className="flex justify-between text-sm font-bold">
                              ...
                              <span style={{ color: 'var(--gold)' }}>{formatCents(order.total_cents)}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          {addr ? (
                            <div className="text-sm space-y-1" style={{ color: 'var(--text-dark)' }}>
                            ...
                            <p className="text-sm" style={{ color: 'var(--text-light)' }}>No address</p>
                          )}

                          {order.patient_notes && (
                            <div className="mt-4">
                              <h4 className="text-xs font-bold uppercase tracking-wide mb-1" style={{ color: 'var(--text-light)' }}>
                                Customer Notes
                              </h4>
                              <p className="text-sm" style={{ color: 'var(--text-dark)' }}>{order.patient_notes}</p>
                            </div>
                          )}
                        </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div>
                              ...
                              <TrackingInput
                                ...
                                onSave={(val) => updateOrder(order.id, { trackingNumber: val })}
                                disabled={updating === order.id}
                              />
                            </div>
                            ...
                            <div>
                              ...
                              <NotesInput
                                initial={order.clinician_notes || ''}
                                onSave={(val) => updateOrder(order.id, { clinicianNotes: val })}
                                disabled={updating === order.id}
                              />
                            </div>
                          </div>
  ...
  return (
    <div className="flex gap-2">
      <input
        ...
        className="flex-1 py-2 px-3 text-sm rounded-lg focus:outline-none"
        style={{ background: 'var(--off-white)', border: '1px solid var(--border)', color: 'var(--text-dark)' }}
      />
      {dirty && (
        <button
  ...
  return (
    <div>
      <textarea
        ...
        rows={2}
        className="w-full py-2 px-3 text-sm rounded-lg focus:outline-none resize-none"
        style={{ background: 'var(--off-white)', border: '1px solid var(--border)', color: 'var(--text-dark)' }}
      />
      {dirty && (
        <button
          onClick={() => { onSave(value); setDirty(false); }}
          disabled={disabled}
          className="mt-1 px-3 py-1.5 rounded-lg text-xs font-bold text-white"
          style={{ background: 'var(--gold)' }}
        >
          Save Notes
        </button>
