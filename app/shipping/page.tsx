        ...
        <div className="container-xl max-w-3xl">
          <div className="space-y-10">
            {content.sections.map((section: ShippingSection, i: number) => (
              <div key={i}>
                {/* Section title — larger for top-level headings like "Payment Information" */}
                    ...
                    {section.paragraphs.map((p: string, j: number) => (
                      <p
                        ...
                        dangerouslySetInnerHTML={{
                          __html: p.replace(
                            /\*\*(.*?)\*\*/g,
                            '<strong class="underline" style="color: var(--navy); font-weight: 700;">$1</strong>'
                          ),
                        }}
                      />
                    ))}
                          ...
                          >
                            {row.map((cell: string, k: number) => (
                              <td
                                key={k}
                                className="px-4 py-3"
                                style={{ color: k === 2 ? 'var(--gold)' : 'var(--text-mid)' }}
                              >
                                {cell}
                              </td>
                            ))}
                ...
                {/* Extra text */}
                {section.extra && (
                  <p className="text-sm leading-relaxed mt-4" style={{ color: 'var(--text-mid)' }}>
                    {section.extra}
                  </p>
                )}
              </div>
